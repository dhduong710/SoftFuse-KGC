...
# Rule sensitivity E2E inference.
...
DST_CKPT="outputs/sensitivity/rule_sensitivity/e2e_checkpoint"

KGE_PATH="dataset/setting_a/backbone_ready/entity_embeddings_rgcn.pt"
OUT_DIR="outputs/sensitivity/rule_sensitivity"
...
# Copy checkpoint into Week25 folder so infer.py writes outputs under outputs/sensitivity/rule_sensitivity.
...
for variant in "${VARIANTS[@]}"; do
  DATASET_PATH="dataset/setting_a/rule_sensitivity/${variant}"
  ...
  for split in "${SPLITS[@]}"; do
    suffix="rule_sensitivity_${variant}"

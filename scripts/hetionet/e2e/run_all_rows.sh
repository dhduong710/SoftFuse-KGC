...
mkdir -p outputs/hetionet/logs outputs/hetionet/e2e_hetionet
...
run_row () {
  ...
  local out_dir="outputs/hetionet/e2e_hetionet/${row_name}"
  ...
  if [[ -f "${ckpt}/adapter_config.json" && -f "${ckpt}/graph_model.bin" ]]; then
    ...
    python main.py \
      ...
      --report_to none \
      --seed "${SEED}" \
      2>&1 | tee "outputs/hetionet/logs/day7_train_${row_name}.log"
  fi
  ...
  for split in valid test; do
    ...
    python infer.py \
      ...
      2>&1 | tee "outputs/hetionet/logs/day7_infer_${row_name}_${split}.log"
  done
}

run_row "backbone_raw" "dataset/setting_d_hetionet/softfuse_ready"
run_row "soft_support_raw" "dataset/setting_d_hetionet/e2e_soft_support_ready"
run_row "fuzzy_retrieval_main" "dataset/setting_d_hetionet/e2e_fuzzy_retrieval_ready"

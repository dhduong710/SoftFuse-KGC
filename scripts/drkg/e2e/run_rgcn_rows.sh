...
mkdir -p outputs/drkg/logs outputs/drkg/e2e_drkg_rgcn
...
run_row () {
  ...
  local out_dir="outputs/drkg/e2e_drkg_rgcn/${row_name}"
  ...
  if [[ -f "${ckpt}/adapter_config.json" && -f "${ckpt}/graph_model.bin" ]]; then
    ...
    python main.py \
      ...
      --report_to none \
      --seed "${SEED}" \
      2>&1 | tee "outputs/drkg/logs/day8_train_${row_name}.log"
  fi
  ...
  for split in valid test; do
    ...
    python infer.py \
      ...
      2>&1 | tee "outputs/drkg/logs/day8_infer_${row_name}_${split}.log"
  done
}

run_row "backbone_raw" "dataset/setting_e_drkg/softfuse_ready/rgcn"
run_row "soft_support_sweep" "dataset/setting_e_drkg/e2e_soft_support_ready/rgcn_sweep_selected"
run_row "fuzzy_retrieval_main" "dataset/setting_e_drkg/e2e_fuzzy_retrieval_ready/rgcn"
